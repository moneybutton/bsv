...
function inject(deps) {
  ...
  Server.prototype.onNetchannel = function(netchannel) {
    let connection = Connection({}, netchannel);
    connection.monitor();
    this.awaits.forEach(function(await) {
      await.resolve(netchannel);
    });
    let promise = this.netchannels.next().value;
    return promise ? promise.then(this.onNetchannel.bind(this)) : undefined;
  };
  ...
  Server.prototype.monitor = function() {
    ...
    let promise = this.netchannels.next().value;
    if (promise)
      promise.then(this.onNetchannel.bind(this));
    return this;
  };
