...
function inject(deps) {
  ...
  function Server(opts, networks, connections, channels, awaits) {
    if (!(this instanceof Server))
      return new Server(opts, networks, connections, channels, awaits);
    this.initialize();
    this.fromObject({
      opts: opts,
      networks: networks,
  ...
  Server.prototype.monitor = function() {
    if (!this.channels)
      this.channels = this.networks[0].awaitChannels();
    let promise = this.channels.next().value;
    if (promise)
