...
let defaultConstants = require('./constants').Default.Bip32
...
class Bip32 extends Struct {
  ...
  fromSeed (bytes) {
    ...
    let hash = Hash.sha512Hmac(bytes, Buffer.from('Bitcoin seed'))
  ...
  async asyncFromSeed (bytes) {
    let workersResult = await Workers.asyncObjectMethod(this, 'fromSeed', [
      bytes
    ])
    return this.fromFastBuffer(workersResult.resbuf)
  }
  ...
  fromBuffer (buf) {
    ...
    let isPrivate = this.versionBytesNum === this.Constants.privKey
    let isPublic = this.versionBytesNum === this.Constants.pubKey
  ...
  async asyncDerive (path) {
    let workersResult = await Workers.asyncObjectMethod(this, 'derive', [
      path
    ])
   ...
   */
  async asyncToString () {
    let workersResult = await Workers.asyncObjectMethod(this, 'toString', [])
...
Bip32.Mainnet = require('./mainnet-bip-32')
Bip32.Testnet = require('./testnet-bip-32')
