 ...
 */
Interp.prototype.checkSignatureEncoding = function(buf) {
  if ((this.flags & (Interp.SCRIPT_VERIFY_DERSIG | Interp.SCRIPT_VERIFY_LOW_S | Interp.SCRIPT_VERIFY_STRICTENC)) != 0 && !Signature.isTxDER(buf)) {
    this.errstr = "SCRIPT_ERR_SIG_DER";
    return false;
  } else {
    var sig = Signature().fromTx(buf);
    if ((this.flags & Interp.SCRIPT_VERIFY_LOW_S) != 0 && !sig.hasLowS()) {
      this.errstr = "SCRIPT_ERR_SIG_DER";
      return false;
    } else if ((this.flags & Interp.SCRIPT_VERIFY_STRICTENC) != 0 && !sig.hasDefinedHashtype()) {
...
Interp.prototype.step = function() {
  ...
  switch (opcodenum)
  {
    ...
    {
      // ( -- value)
      // CScriptNum bn((int)opcode - (int)(Opcode.OP_1 - 1));
    ...
    case Opcode.OP_NOP1: case Opcode.OP_NOP2: case Opcode.OP_NOP3: case Opcode.OP_NOP4: case Opcode.OP_NOP5:
    ...
    case Opcode.OP_ROLL:
    {
      ...
      var bn = BN().fromCScriptNumBuffer(buf, fRequireMinimal);
      var n = bn.toNumber();
      this.stack.pop();
    ...
    case Opcode.OP_0NOTEQUAL:
    {
      ...
      switch (opcodenum)
      {
      case Opcode.OP_1ADD:     bn = bn.add(1); break;
      case Opcode.OP_1SUB:     bn = bn.sub(1); break;
      case Opcode.OP_NEGATE:   bn = bn.neg(); break;
      case Opcode.OP_ABS:    if (bn.cmp(0) < 0) bn = bn.neg(); break;
      case Opcode.OP_NOT:    bn = BN((bn.cmp(0) === 0) + 0); break;
      case Opcode.OP_0NOTEQUAL:  bn = BN((bn.cmp(0) !== 0) + 0); break;
    ...
    case Opcode.OP_MAX:
    {
      ...
      var bn1 = BN().fromCScriptNumBuffer(this.stack[this.stack.length - 2], fRequireMinimal);
      var bn2 = BN().fromCScriptNumBuffer(this.stack[this.stack.length - 1], fRequireMinimal);
      var bn = BN(0);
      ...
      case Opcode.OP_BOOLAND:       bn = BN(((bn1.cmp(0) !== 0) && (bn2.cmp(0) !== 0)) + 0); break;
      // case Opcode.OP_BOOLOR:        bn = (bn1 != bnZero || bn2 != bnZero); break;
      case Opcode.OP_BOOLOR:        bn = BN(((bn1.cmp(0) !== 0) || (bn2.cmp(0) !== 0)) + 0); break;
      // case Opcode.OP_NUMEQUAL:      bn = (bn1 == bn2); break;
      case Opcode.OP_NUMEQUAL:      bn = BN((bn1.cmp(bn2) === 0) + 0); break;
      // case Opcode.OP_NUMEQUALVERIFY:    bn = (bn1 == bn2); break;
      case Opcode.OP_NUMEQUALVERIFY:    bn = BN((bn1.cmp(bn2) === 0) + 0); break;
      // case Opcode.OP_NUMNOTEQUAL:     bn = (bn1 != bn2); break;
      case Opcode.OP_NUMNOTEQUAL:     bn = BN((bn1.cmp(bn2) !== 0) + 0); break;
      // case Opcode.OP_LESSTHAN:      bn = (bn1 < bn2); break;
      case Opcode.OP_LESSTHAN:      bn = BN((bn1.cmp(bn2) < 0) + 0); break;
      // case Opcode.OP_GREATERTHAN:     bn = (bn1 > bn2); break;
      case Opcode.OP_GREATERTHAN:     bn = BN((bn1.cmp(bn2) > 0) + 0); break;
      // case Opcode.OP_LESSTHANOREQUAL:   bn = (bn1 <= bn2); break;
      case Opcode.OP_LESSTHANOREQUAL:   bn = BN((bn1.cmp(bn2) <= 0) + 0); break;
      // case Opcode.OP_GREATERTHANOREQUAL:  bn = (bn1 >= bn2); break;
      case Opcode.OP_GREATERTHANOREQUAL:  bn = BN((bn1.cmp(bn2) >= 0) + 0); break;
      case Opcode.OP_MIN:         bn = (bn1.cmp(bn2) < 0 ? bn1 : bn2); break;
      case Opcode.OP_MAX:         bn = (bn1.cmp(bn2) > 0 ? bn1 : bn2); break;
      // default:           assert(!"invalid opcode"); break; //TODO: does this ever occur?
      }
      this.stack.pop();
      this.stack.pop();
    ...
    case Opcode.OP_WITHIN:
    {
      ...
      var bn1 = BN().fromCScriptNumBuffer(this.stack[this.stack.length - 3], fRequireMinimal);
      var bn2 = BN().fromCScriptNumBuffer(this.stack[this.stack.length - 2], fRequireMinimal);
      var bn3 = BN().fromCScriptNumBuffer(this.stack[this.stack.length - 1], fRequireMinimal);
    ...
    case Opcode.OP_CHECKSIGVERIFY:
    {
      ...
      if (!this.checkSignatureEncoding(vchSig) || !this.checkPubkeyEncoding(vchPubkey)) {
        // serror is set
        return false;
      }

      var sig = Signature().fromTx(vchSig);
      var pubkey = Pubkey().fromBuffer(vchPubkey, false);
    ...
    case Opcode.OP_CHECKMULTISIGVERIFY:
    {
      ...
      var nKeysCount = BN().fromCScriptNumBuffer(this.stack[this.stack.length - i], fRequireMinimal).toNumber();
      ...
      var nSigsCount = BN().fromCScriptNumBuffer(this.stack[this.stack.length - i], fRequireMinimal).toNumber();
...
Interp.prototype.verify = function(scriptSig, scriptPubkey, tx, nin, flags) {
  ...
  if ((flags & Interp.SCRIPT_VERIFY_P2SH) && scriptPubkey.isScripthashOut())
  {
    ...
    if (!Interp.castToBool(stackCopy[stackCopy.length - 1])) {
      this.errstr = "SCRIPT_ERR_EVAL_FALSE";
      return false;
    } else {
      return true;
