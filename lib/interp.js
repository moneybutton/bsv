...
let Interp = function Interp(stack, altstack, pc, pbegincodehash, nOpCount, ifstack, errstr, flags) {
  if (!(this instanceof Interp))
    return new Interp(stack, altstack, pc, pbegincodehash, nOpCount, ifstack, errstr, flags);
  this.initialize();
  this.fromObject({
    stack: stack,
    altstack: altstack,
...
Interp.prototype.fromJSON = function(json) {
  this.fromObject({
    ...
    tx: json.tx ? Tx().fromJSON(json.tx) : undefined,
    nin: json.nin
  });
  this.stack = [];
...
Interp.prototype.toJSON = function() {
  ...
  return {
    ...
    tx: this.tx ? this.tx.toJSON() : undefined,
    nin: this.nin,
    stack: stack,
    altstack: altstack,
    pc: this.pc,
...
/**
 * Translated from bitcoind's CheckSigEncoding
 */
...
Interp.prototype.step = function() {
  ...
  switch (opcodenum)
  {
    ...
    case Opcode.OP_0NOTEQUAL:
    {
      ...
      case Opcode.OP_ABS:    if (bn.lt(0)) bn = bn.neg(); break;
      case Opcode.OP_NOT:    bn = BN(bn.eq(0) + 0); break;
      case Opcode.OP_0NOTEQUAL:  bn = BN(bn.neq(0) + 0); break;
    ...
    case Opcode.OP_MAX:
    {
      ...
      case Opcode.OP_BOOLAND:       bn = BN((bn1.neq(0) && bn2.neq(0)) + 0); break;
      // case Opcode.OP_BOOLOR:        bn = (bn1 != bnZero || bn2 != bnZero); break;
      case Opcode.OP_BOOLOR:        bn = BN((bn1.neq(0) || bn2.neq(0)) + 0); break;
      // case Opcode.OP_NUMEQUAL:      bn = (bn1 == bn2); break;
      case Opcode.OP_NUMEQUAL:      bn = BN(bn1.eq(bn2) + 0); break;
      // case Opcode.OP_NUMEQUALVERIFY:    bn = (bn1 == bn2); break;
      case Opcode.OP_NUMEQUALVERIFY:    bn = BN(bn1.eq(bn2) + 0); break;
      // case Opcode.OP_NUMNOTEQUAL:     bn = (bn1 != bn2); break;
      case Opcode.OP_NUMNOTEQUAL:     bn = BN(bn1.neq(bn2) + 0); break;
      // case Opcode.OP_LESSTHAN:      bn = (bn1 < bn2); break;
      case Opcode.OP_LESSTHAN:      bn = BN(bn1.lt(bn2) + 0); break;
      // case Opcode.OP_GREATERTHAN:     bn = (bn1 > bn2); break;
      case Opcode.OP_GREATERTHAN:     bn = BN(bn1.gt(bn2) + 0); break;
      // case Opcode.OP_LESSTHANOREQUAL:   bn = (bn1 <= bn2); break;
      case Opcode.OP_LESSTHANOREQUAL:   bn = BN(bn1.leq(bn2) + 0); break;
      // case Opcode.OP_GREATERTHANOREQUAL:  bn = (bn1 >= bn2); break;
      case Opcode.OP_GREATERTHANOREQUAL:  bn = BN(bn1.geq(bn2) + 0); break;
      case Opcode.OP_MIN:         bn = (bn1.lt(bn2) ? bn1 : bn2); break;
      case Opcode.OP_MAX:         bn = (bn1.gt(bn2) ? bn1 : bn2); break;
      // default:           assert(!"invalid opcode"); break; //TODO: does this ever occur?
      }
      this.stack.pop();
      this.stack.pop();
...
Interp.prototype.verify = function(scriptSig, scriptPubkey, tx, nin, flags) {
  ...
  if ((flags & Interp.SCRIPT_VERIFY_P2SH) && scriptPubkey.isScripthashOut())
  {
    ...
    let pubkeySerialized = stackCopy[stackCopy.length - 1];
