...
import { TxIn } from './tx-in'
  ...
  class Interp extends Struct {
    ...
    this.nonTopLevelReturnAfterGenesis = false;
      ...
      try {
        while (this.pc < this.script.chunks.length) {
          ...
          const execStep = {stack: Array.from(this.stack),  alt: Array.from(this.altStack), errStr: this.errStr, remainingChunks: this.script.chunks.slice(this.pc), pc: this.pc }
          // console.log({r:s.remainingChunks, errStr: this.errStr})
          this?.execSteps?.push(execStep)
          if (!fSuccess) {
            yield false
          } else {
            yield fSuccess
      ...
      if (fExec && opCodeNum >= 0 && opCodeNum <= OpCode.OP_PUSHDATA4) {
        ...
        switch (opCodeNum) {
          ...
          case OpCode.OP_NUM2BIN:
            ...
            var size = new Bn().fromScriptNumBuffer(this.stack[this.stack.length -1], fRequireMinimal).toNumber()
          ...
          case OpCode.OP_BIN2NUM:
            ...
            buf1 = this.stack[this.stack.length -1]
            buf2 = Interp._minimallyEncode(buf1)
    
            this.stack[this.stack.length - 1] = buf2
    
            // The resulting number must be a valid number.
            if (!Interp._isMinimallyEncoded(buf2)) {
              this.errstr = 'SCRIPT_ERR_INVALID_NUMBER_RANGE'
              return false
            }
            break
    ...
    * results (scriptSig, scriptPubKey, tx, nIn, flags, valueBn) {
    ...
    /**
      ...
      */
      Interp._minimallyEncode = function (buf) {
        if (buf.length === 0) {
          return buf
        }
      ...
      Interp._isMinimallyEncoded = function (buf, nMaxNumSize) {
        nMaxNumSize = nMaxNumSize || Interp.MAXIMUM_ELEMENT_SIZE
  ...
  Interp.MAX_SCRIPT_ELEMENT_SIZE = 100000000
  ...
  // not.
  Interp.defaultFlags =
    Interp.SCRIPT_VERIFY_P2SH | Interp.SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY | Interp.SCRIPT_VERIFY_CLEANSTACK 
