...
let inject = function (deps) {
  ...
  class Interp extends Struct {
    constructor (script, tx, nIn, stack, altStack, pc, pBeginCodeHash, nOpCount, ifStack, errStr, flags) {
      super()
      this.initialize()
...
/**
    ...
    * eval () {
          ...
          case OpCode.OP_0NOTEQUAL:
            {
              ...
              switch (opCodeNum) {
                ...
                case OpCode.OP_ABS: if (bn.lt(0)) bn = bn.neg()
                  break
                case OpCode.OP_NOT: bn = new Bn(bn.eq(0) + 0)
                  break
                case OpCode.OP_0NOTEQUAL: bn = new Bn(bn.neq(0) + 0)
                  break
          ...
          case OpCode.OP_MAX:
            {
              ...
              switch (opCodeNum) {
                ...
                case OpCode.OP_BOOLAND: bn = new Bn((bn1.neq(0) && bn2.neq(0)) + 0)
                  break
                // case OpCode.OP_BOOLOR:        bn = (bn1 != bnZero || bn2 != bnZero); break
                case OpCode.OP_BOOLOR: bn = new Bn((bn1.neq(0) || bn2.neq(0)) + 0)
                  break
                // case OpCode.OP_NUMEQUAL:      bn = (bn1 == bn2); break
                case OpCode.OP_NUMEQUAL: bn = new Bn(bn1.eq(bn2) + 0)
                  break
                // case OpCode.OP_NUMEQUALVERIFY:    bn = (bn1 == bn2); break
                case OpCode.OP_NUMEQUALVERIFY: bn = new Bn(bn1.eq(bn2) + 0)
                  break
                // case OpCode.OP_NUMNOTEQUAL:     bn = (bn1 != bn2); break
                case OpCode.OP_NUMNOTEQUAL: bn = new Bn(bn1.neq(bn2) + 0)
                  break
                // case OpCode.OP_LESSTHAN:      bn = (bn1 < bn2); break
                case OpCode.OP_LESSTHAN: bn = new Bn(bn1.lt(bn2) + 0)
                  break
                // case OpCode.OP_GREATERTHAN:     bn = (bn1 > bn2); break
                case OpCode.OP_GREATERTHAN: bn = new Bn(bn1.gt(bn2) + 0)
                  break
                // case OpCode.OP_LESSTHANOREQUAL:   bn = (bn1 <= bn2); break
                case OpCode.OP_LESSTHANOREQUAL: bn = new Bn(bn1.leq(bn2) + 0)
                  break
                // case OpCode.OP_GREATERTHANOREQUAL:  bn = (bn1 >= bn2); break
                case OpCode.OP_GREATERTHANOREQUAL: bn = new Bn(bn1.geq(bn2) + 0)
                  break
          ...
          case OpCode.OP_CHECKSIGVERIFY:
            {
              ...
              let subScript = new Script().fromObject({chunks: this.script.chunks.slice(this.pBeginCodeHash)})
     ...
     */
    verify (scriptSig, scriptPubKey, tx, nIn, flags) {
     ...
     */
    * results (scriptSig, scriptPubKey, tx, nIn, flags) {
      let stackCopy
     ...
     * information about exactly where the script failed. pc refers to the
    ...
    getFailureExplanation () {
      ...
      return {
        ...
        script: this.script,
        pc: pc,
        opCode: OpCode.fromNumber(this.script.chunks[pc].opCodeNum)
      }
    }

    getFailureExplanationString () {
      let obj = this.getFailureExplanation()
      return JSON.stringify({
        errStr: obj.errStr,
        script: obj.script.toString(),
        pc: obj.pc,
        opCode: obj.opCode.toString()
      })
    }
  }
