...
let inject = function (deps) {
   ...
   */
  Interp.prototype.checkSequence = function (seqnum) {
    // Relative lock times are supported by comparing the passed
    // in operand to the sequence number of the input.
    let txToSequence = this.tx.txins[this.nin].seqnum
    ...
    let nSequenceMasked = seqnum & nLockTimeMask
        ...
        case Opcode.OP_CHECKSEQUENCEVERIFY:
          {
            ...
            let seqnumbuf = this.stack[this.stack.length - 1]
            let seqnumbn = BN().fromScriptNumBuffer(seqnumbuf, fRequireMinimal, 5)
            let seqnum = seqnumbn.toNumber()
            ...
            if ((seqnum & Txin.SEQUENCE_LOCKTIME_DISABLE_FLAG) !== 0) {
              break
            }

            // Compare the specified sequence number with the input.
            if (!this.checkSequence(seqnum)) {
              this.errstr = 'SCRIPT_ERR_UNSATISFIED_LOCKTIME'
              return false
            }
          }
