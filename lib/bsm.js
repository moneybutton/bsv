...
let inject = function (deps) {
  ...
  class Bsm extends Struct {
    constructor (messageBuf, keyPair, sig, address, verified) {
      super()
      this.fromObject({messageBuf, keyPair, sig, address, verified})
    }
    ...
    sign () {
      ...
      let ecdsa = new Ecdsa().fromObject({hashBuf: hashBuf, keyPair: this.keyPair})
      ecdsa.sign()
      ecdsa.calcrecovery()
      this.sig = ecdsa.sig
      return this
    ...
    verify () {
      ...
      let address = new Address().fromPubKey(ecdsa.keyPair.pubKey, undefined, this.sig.compressed)
