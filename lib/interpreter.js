...
var Interpreter = function Interpreter(script, tx, nin) {
  if (!(this instanceof Interpreter))
    return new Interpreter(script, tx, nin);
...
// like bitcoind's CheckMinimalPush ... but the logic is in script.js
Interpreter.checkMinimalPush = function checkMinimalPush(buf, opcodenum) {
  return Script({
    chunks: [{
      opcodenum: opcodenum,
      buf: buf,
      len: typeof buf !== 'undefined' ? buf.length : undefined
    }]
  }).checkMinimalPush(0);
};
...
Interpreter.prototype.step = function() {
  ...
  if (chunk.buf && chunk.buf.length > Script.MAX_SCRIPT_ELEMENT_SIZE) {
  ...
  if (fExec && 0 <= opcodenum && opcodenum <= Opcode.map.OP_PUSHDATA4) {
    if (fRequireMinimal && !Interpreter.checkMinimalPush(vchPushValue, opcode)) {
  ...
  switch (opcodenum)
  {
    ...
    case Opcode.map.OP_16:
    {
      ...
      var buf = BN(n).toSM({endian: 'little'});
      this.stack.push(buf);
    ...
    case Opcode.map.OP_NOTIF:
    {
      ...
      if (fExec)
      {
        ...
        fValue = BN().fromSM(vch, {endian: 'little'}).cmp(0) === 0;
    ...
    case Opcode.map.OP_VERIFY:
    {
      ...
      var fValue = BN().fromSM(vch, {endian: 'little'}).cmp(0) === 0;
      if (fValue)
        this.stack.pop();
      else {
    ...
    {
      // -- stacksize
      var vch = BN(this.stack.length).toSM({endian: 'little'});
      this.stack.push(vch);
    }
    break;
    ...
    case Opcode.map.OP_ROLL:
    {
      ...
      var bn = BN().fromSM(buf, {endian: 'little'});
      this.stack.pop();
    ...
    case Opcode.map.OP_SIZE:
    {
      ...
      this.stack.push(bn.toSM({endian: 'little'}));
    }
    break;
    ...
    case Opcode.map.OP_MAX:
    {
      ...
      // CScriptNum bn(0);
      var bn1 = BN().fromSM(this.stack[this.stack.length - 2], {endian: 'little'});
      var bn2 = BN().fromSM(this.stack[this.stack.length - 1], {endian: 'little'});
      var bn = BN(0);
    ...
    case Opcode.map.OP_WITHIN:
    {
      ...
      var b1 = BN().fromSM(this.stack[this.stack.length - 3], {endian: 'little'});
      var b2 = BN().fromSM(this.stack[this.stack.length - 2], {endian: 'little'});
      var b3 = BN().fromSM(this.stack[this.stack.length - 1], {endian: 'little'});
    ...
    case Opcode.map.OP_CHECKSIGVERIFY:
    {
      ...
      var subscript = Script().set({chunks: this.script.slice(this.pbegincodehash)});
      ...
      var fSuccess = tx.verify(sig, pubkey, this.nin, subscript);
    ...
    case Opcode.map.OP_CHECKMULTISIGVERIFY:
    {
      ...
      var nKeysCount = BN().fromSM(this.stack[this.stack.length - i], {endian: 'little'}).toNumber();
      ...
      var nSigsCount = BN().fromSM(this.stack[this.stack.length - 1], {endian: 'little'}).toNumber();
      ...
      while (fSuccess && nSigsCount > 0)
      {
        ...
        var sig = Signature().fromTx(vchSig);
        var pubkey = Pubkey().fromBuffer(vchPubkey);
        var fOk = tx.verify(sig, pubkey, this.nin, subscript);
...
Interpreter.prototype.verify = function(scriptSig, scriptPubkey, tx, nin, flags) {
  ...
  var buf = this.stack[this.stack.length - 1];
  if (BN().fromSM(buf, {endian: 'little'}).cmp(0) === 0) {
    this.errstr = "SCRIPT_ERR_EVAL_FALSE";
    return false;
  }
