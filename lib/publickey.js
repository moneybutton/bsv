...
/**
 * Will return child PublicKey derived from id by IBE.
 *
 * @param {String} ID
 * @returns {PublicKey} A new valid instance of IBE child PublicKey
 */
PublicKey.prototype.childKey = function (id) {
  var idbuf = Buffer.from(id)
  var hash = Hash.sha512hmac(Buffer.from(id), Buffer.from(this.toString()))
  var leftpart = BN.fromBuffer(hash.slice(0, 32))
  // OR if(PublicKey.isValid(Point.getG().mul(leftpart).add(this.point)))
  try {
    var newpublicKey = PublicKey.fromPoint(Point.getG().mul(leftpart).add(this.point))
  } catch (e) {
    return this.childKey(Buffer.concat([idbuf, Buffer.from('\0')]))
  }
  return newpublicKey
}
