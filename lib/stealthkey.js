...
StealthKey.prototype.fromObject = function(obj) {
...
StealthKey.prototype.getReceiveKeypair = function(senderPubkey) {
  let sharedKeypair = this.getSharedKeypair(senderPubkey);
  let privkey = Privkey({bn: this.payloadKeypair.privkey.bn.add(sharedKeypair.privkey.bn).mod(Point.getN())});
  let key = Keypair({privkey: privkey});
  key.privkey2pubkey();
