...
Hash.sha1 = function(buf) {
  ...
  var hashbuf = crypto.createHash('sha1').update(buf).digest();
  return hashbuf;
};
...
Hash.sha256 = function(buf) {
  ...
  var hashbuf = crypto.createHash('sha256').update(buf).digest();
  return hashbuf;
};
...
Hash.sha512 = function(buf) {
  ...
  var hashbuf = crypto.createHash('sha512').update(buf).digest();
  return hashbuf;
};
...
Hash.sha1hmac = function(data, key) {
  return Hash.hmac(Hash.sha1, data, key);
};
