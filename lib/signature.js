...
var Signature = function Signature(r, s, nhashtype) {
  if (!(this instanceof Signature))
    return new Signature(r, s, nhashtype);
  if (r instanceof BN) {
    this.set({
      r: r,
      s: s,
...
Signature.SIGHASH_ALL = 0x00000001;
Signature.SIGHASH_NONE = 0x00000002;
Signature.SIGHASH_SINGLE = 0x00000003;
Signature.SIGHASH_ANYONECANPAY = 0x00000080;
...
Signature.prototype.fromTxBuffer = function(buf) {
  console.log(buf.toString('hex'));
  var nhashtype = buf.readInt32LE(buf.length - 4);
  var derbuf = buf.slice(0, buf.length - 4);
  console.log(derbuf.toString('hex'));
  this.fromDER(derbuf);
...
Signature.prototype.toTxBuffer = function() {
  var derbuf = this.toDER();
  var buf = new Buffer(4);
  buf.writeInt32LE(this.nhashtype, 0);
