...
let defaultWordlist = require('./bip-39-en-wordlist')
let Workers = require('./workers')

class Bip39 extends Struct {
  constructor (mnemonic, seed, wordlist = defaultWordlist) {
    super({ mnemonic, seed })
  ...
  check () {
    ...
    let words = mnemonic.split(this.Wordlist.space)
    let bin = ''
    for (let i = 0; i < words.length; i++) {
      let ind = this.Wordlist.indexOf(words[i])
      if (ind < 0) {
        return false
      }
...
/*
  The need to export these other languages here is *questionable* at best.

  This will ensure that whenever new Bip39 languages are added in the future,
  all of them are always included in bundles that import Bip39. It may be
  better to simply require users to import lib/bip-39-xx when they wish to use
  a non-default language.

  it would also be nice to be able to inject a default Bip39 language, similar
  to how we inject the default NETWORK variable.

  Additionally, Removal of these exports would allow as many new languages as
  desired to be added without unduly impacting final build output size.
*/
Bip39.En = require('./bip-39-en')
Bip39.Jp = require('./bip-39-jp')
