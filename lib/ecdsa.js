  ...
  class Ecdsa extends Struct {
    ...
    sign () {
      ...
      if (process.env.BSV_USE_LIB_SECP256K1 === 'true') {
        let rsbuf
        try {
          rsbuf = secp256k1.sign(
            hashBuf,
            this.keyPair.privKey.bn.toBuffer({ size: 32 })
          ).signature
        } catch (err) {
          if (err.message === 'message length is invalid') {
            throw new Error('hashBuf must be a 32 byte buffer')
          } else {
            throw err
          }
        }
        let sig = Sig.fromRS(rsbuf)
        sig.compressed = this.keyPair.pubKey.compressed
        this.sig = sig
        return this
      }
    ...
    verify (enforceLowS = true) {
      if (
        enforceLowS &&
        process.env.BSV_USE_LIB_SECP256K1 === 'true'
      ) {
        let hashBuf =
          this.endian === 'little'
            ? new Br(this.hashBuf).readReverse()
