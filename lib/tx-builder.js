...
class TxBuilder extends Struct {
  ...
  inputFromScriptHashMultiSig (
    ...
    if (
      !Buffer.isBuffer(txHashBuf) ||
        !(typeof txOutNum === 'number') ||
        !(txOut instanceof TxOut) ||
        !(redeemScript instanceof Script)
    ) {
      throw new Error(
        'invalid one of: txHashBuf, txOutNum, txOut, redeemScript'
      )
    }
    this.txIns.push(
      new TxIn()
  ...
  fillScriptHashMultiSigSig (i, keyPair, sig, redeemScript) {
    ...
    let pubKeychunks = redeemScript.chunks.slice(
      1,
      redeemScript.chunks.length - 2
    )
    ...
    for (
      thisPubKeyIndex = 0;
      thisPubKeyIndex < pubKeybufs.length;
      thisPubKeyIndex++
    ) {
    ...
    script.chunks[thisPubKeyIndex + 1] = new Script().writeBuffer(
      sig.toTxFormat()
    ).chunks[0]
  ...
  getSig (keyPair, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, nIn, subScript, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID) {
    ...
    return this.tx.sign(keyPair, nHashType, nIn, subScript, valueBn, flags)
  }
  ...
  sign (i, keyPair, txOut, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID) {
    ...
    if (script.isPubKeyHashIn()) {
      ...
      let sig = this.getSig(keyPair, nHashType, i, subScript, flags)
      this.fillPubKeyHashSig(i, keyPair, sig, subScript)
    } else if (script.isScriptHashIn()) {
      let redeemScript = new Script().fromBuffer(
        script.chunks[script.chunks.length - 1].buf
      )
      let subScript = redeemScript
      if (!redeemScript.isMultiSigOut()) {
        throw new Error(
          'cannot sign non-multisig scriptHash script type for input ' + i
        )
      }
      let sig = this.getSig(keyPair, nHashType, i, subScript, flags)
