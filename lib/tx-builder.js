...
let SigOperations = require('./sig-operations')
let Sig = require('./sig')
...
class TxBuilder extends Struct {
  ...
  toJSON () {
    ...
    json.changeAmount = this.changeAmount ? this.changeAmount.toNumber() : undefined
    json.feeAmount = this.feeAmount ? this.feeAmount.toNumber() : undefined
  ...
  fromJSON (json) {
    ...
    this.changeAmount = json.changeAmount ? new Bn(json.changeAmount) : undefined
    this.feeAmount = json.feeAmount ? new Bn(json.feeAmount) : undefined
  ...
  inputFromPubKeyHash (txHashBuf, txOutNum, txOut, pubKey, nSequence, nHashType) {
    ...
    this.uTxOutMap.set(txHashBuf, txOutNum, txOut)
    let addressStr = Address.fromTxOutScript(txOut.script).toString()
  ...
  estimateSize () {
    ...
    this.tx.txIns.forEach((txIn) => {
      let { txHashBuf, txOutNum } = txIn
      let sigOperations = this.sigOperations.get(txHashBuf, txOutNum)
      sigOperations.forEach((obj) => {
        let { nScriptChunk, type } = obj
        let script = new Script([txIn.script.chunks[nScriptChunk]])
        let scriptSize = script.toBuffer().length
        size -= scriptSize
        if (type === 'sig') {
          size += sigSize
  ...
  build (opts = { useAllInputs: false }) {
    ...
    for (
      ...
      this.changeAmount = inAmount.sub(outAmount)
      this.tx.txOuts[this.tx.txOuts.length - 1].valueBn = this.changeAmount

      minFeeAmount = this.estimateFee()
      if (
        this.changeAmount.geq(minFeeAmount) &&
          this.changeAmount.sub(minFeeAmount).gt(this.dust)
      ) {
        break
      }
    }
    if (this.changeAmount.geq(minFeeAmount)) {
      // Subtract fee from change
      this.feeAmount = minFeeAmount
      this.changeAmount = this.changeAmount.sub(this.feeAmount)
      this.tx.txOuts[this.tx.txOuts.length - 1].valueBn = this.changeAmount

      if (this.changeAmount.lt(this.dust)) {
        if (this.dustToChangeOrFees) {
          // Remove the change amount since it is less than dust and the
          // builder has requested dust be sent to fees.
          this.tx.txOuts.pop()
          this.feeAmount = this.feeAmount.add(this.changeAmount)
          this.changeAmount = new Bn(0)
        } else {
  ...
  fillSig (txInIndex, nScriptChunk, sig) {
    let txIn = this.tx.txIns[txInIndex]
  ...
  signTxIn (nIn, keyPair, txOut, nScriptChunk, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID) {
    ...
    let sig = this.getSig(keyPair, nHashType, nIn, subScript, flags, this.hashCache)
  ...
  async asyncSignTxIn (nIn, keyPair, txOut, nScriptChunk, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID) {
    ...
    let outScript = txOut.script
    let subScript = outScript // true for standard script types
    let sig = await this.asyncGetSig(keyPair, nHashType, nIn, subScript, flags, this.hashCache)
    this.fillSig(nIn, nScriptChunk, sig)
    return this
  }
  ...
  sign (keyPairs) {
    ...
    for (let keyPair of keyPairs) {
      let addressStr = Address.fromPubKey(keyPair.pubKey).toString()
      addressStrMap[addressStr] = keyPair
    }
    // loop through all inputs
    for (let nIn in this.tx.txIns) {
