...
let inject = function (deps) {
  ...
  class TxBuilder extends Struct {
    constructor (tx = new Tx(), txIns = [], txOuts = [], uTxOutMap = new TxOutMap(), changeScript, feePerKbNum = Constants.feePerKbNum, nLockTime = 0, versionBytesNum = 1) {
      super({tx, txIns, txOuts, uTxOutMap, changeScript, feePerKbNum, nLockTime, versionBytesNum})
    }
    ...
    build (useAllInputs = false) {
      ...
      for (let extraInputsNum = useAllInputs ? this.txIns.length - 1 : 0; extraInputsNum < this.txIns.length; extraInputsNum++) {
        this.tx = new Tx()
