...
class TxBuilder extends Struct {
  ...
  /**
     ...
     */
  signTxIn (i, keyPair, txOut, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID) {
    let txIn = this.tx.txIns[i]
  ...
  /**
     ...
     */
  async asyncSignTxIn (i, keyPair, txOut, nHashType = Sig.SIGHASH_ALL, flags) {
    let txIn = this.tx.txIns[i]
    let script = txIn.script
    if (script.isPubKeyHashIn()) {
      let txHashBuf = txIn.txHashBuf
