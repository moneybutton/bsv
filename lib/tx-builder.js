...
class TxBuilder extends Struct {
    ...
    this.txOuts.forEach(txOut => {
      if (txOut.valueBn.lt(this.dust) && !txOut.script.isOpReturn()) {
        if (this.dustChangeToFees) {
          // This output is smaller than dust, and the dustChangeToFees option
  ...
  buildInputs (outAmountBn, extraInputsNum = 0) {
    ...
    if (inAmountBn.lt(outAmountBn)) {
      throw new Error(
        'not enough funds for output: inAmountBn ' +
            inAmountBn.toNumber() +
            ' outAmountBn ' +
