...
let PubKeyMap = require('./pub-key-map')
...
class TxBuilder extends Struct {
  ...
  /**
     ...
     */
  importPartiallySignedTx (tx, uTxOutMap = this.uTxOutMap, pubKeyMap = this.pubKeyMap) {
    this.tx = tx
  ...
  addSigPubKey (txHashBuf, txOutNum, scriptChunkIndex, pubKey) {
    this.pubKeyMap.set(txHashBuf, txOutNum, scriptChunkIndex, pubKey)
    return this
  }
  ...
  inputFromPubKeyHash (txHashBuf, txOutNum, txOut, pubKey, nSequence) {
    ...
    this.uTxOutMap.set(txHashBuf, txOutNum, txOut)
    this.addSigPubKey(txHashBuf, txOutNum, 0, pubKey)
    return this
  }
  ...
  fillSig (txInIndex, scriptChunkIndex, sig) {
    let txIn = this.tx.txIns[txInIndex]
    txIn.script.chunks[scriptChunkIndex] = new Script().writeBuffer(
      sig.toTxFormat()
    ).chunks[scriptChunkIndex]
  ...
  /**
     ...
     */
  signTxIn (i, keyPair, txOut, scriptChunkIndex, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID) {
    let txIn = this.tx.txIns[i]
    let script = txIn.script
    if (scriptChunkIndex === undefined && script.isPubKeyHashIn()) {
      scriptChunkIndex = 0
    } else {
    ...
    this.fillSig(i, scriptChunkIndex, sig)
    return this
  }
  ...
  /**
     ...
     */
  async asyncSignTxIn (i, keyPair, txOut, scriptChunkIndex, nHashType = Sig.SIGHASH_ALL, flags) {
    let txIn = this.tx.txIns[i]
    let script = txIn.script
    if (scriptChunkIndex === undefined && script.isPubKeyHashIn()) {
      scriptChunkIndex = 0
    ...
    let outScript = txOut.script
    let subScript = outScript // true for standard script types
    let sig = await this.asyncGetSig(keyPair, nHashType, i, subScript, flags, this.hashCache)
    this.fillSig(i, scriptChunkIndex, sig)
    return this
  }
}
