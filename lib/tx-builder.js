...
class TxBuilder extends Struct {
  ...
  addPubKeyForSig (txHashBuf, txOutNum, nScriptChunk, pubKey) {
    this.pubKeyMap.addOne(txHashBuf, txOutNum, nScriptChunk, pubKey)
    return this
  }
  ...
  inputFromPubKeyHash (txHashBuf, txOutNum, txOut, pubKey, nSequence) {
    ...
    this.uTxOutMap.set(txHashBuf, txOutNum, txOut)
    this.addPubKeyForSig(txHashBuf, txOutNum, 0, pubKey)
    return this
  }
  ...
  sign (keyPairs) {
    ...
    for (let keyPair of keyPairs) {
      keyPairMap[keyPair.pubKey.toString()] = keyPair
    }
    // loop through all inputs
    for (let nIn in this.tx.txIns) {
      const txIn = this.tx.txIns[nIn]
      // for each input, use pubKeyMap to get list of signatures to be produced
      const arr = this.pubKeyMap.get(txIn.txHashBuf, txIn.txOutNum)
      for (let obj of arr) {
        // for each pubkey, get the privkey from the privkey map and sign the input
        const { nScriptChunk, pubKey, nHashType } = obj
        const keyPair = keyPairMap[pubKey.toString()]
        if (!keyPair) {
          // console.log('cannot find keyPair for pubKey ', pubKey.toString())
          continue
        }
