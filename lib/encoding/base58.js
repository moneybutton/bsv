...
var bn58 = new BN(58)
var padding = Buffer.from('00', 'hex')
...
Base58.encode = function (buf) {
  ...
  var leadingZero = 0
  while (leadingZero < buf.length && buf[leadingZero] === 0) {
    leadingZero++
  }
  var n = BN.fromBuffer(buf)
  var s = ''
  var pad = ALPHABET[0].repeat(leadingZero)
  while (!BN.Zero.eq(n)) {
    var r = n.divmod(bn58)
    s = ALPHABET[r.mod.toNumber()] + s
    n = r.div
  }
  return pad + s
}
...
Base58.decode = function (str) {
  ...
  while (arr.length > 0) {
    var c = arr.shift()
    if (ALPHABET.indexOf(c) === -1) throw new Error('Non-base58 character')
    var m = new BN(ALPHABET.indexOf(c))
    if (n.eq(BN.Zero) && m.eq(BN.Zero))pad = Buffer.concat([pad, padding])
    n = n.mul(bn58).add(m)
  }
  return Buffer.concat([pad, n.toBuffer()])
}
