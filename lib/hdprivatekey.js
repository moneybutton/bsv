...
HDPrivateKey.fromRandom = async function () {
  return new HDPrivateKey().initialized
}
 ...
 * @param {boolean?} hardened
 */
HDPrivateKey.prototype.deriveChild = async function (arg, hardened) {
  if (!this.instance) throw new Error('Uninitialized HDPrivateKey instance!')
  if (_.isNumber(arg)) {
 ...
 * @param {boolean?} hardened
 */
HDPrivateKey.prototype.deriveNonCompliantChild = async function (arg, hardened) {
  if (!this.instance) throw new Error('Uninitialized HDPrivateKey instance!')
  if (_.isNumber(arg)) {
...
HDPrivateKey.prototype._deriveWithNumber = async function (index, hardened, nonCompliant) {
  ...
  var hash = await Hash.sha512hmac(data, this._buffers.chainCode)
  ...
  var derived = await new HDPrivateKey({
...
HDPrivateKey.prototype._deriveFromString = async function (path, nonCompliant) {
  ...
  for (index in indexes) {
    derived = await derived._deriveWithNumber(index, null, nonCompliant)
  }
 ...
 */
HDPrivateKey.getSerializedError = async function (data, network) {
  ...
  try {
    data = await Base58Check.decode(data)
  } catch (e) {
...
HDPrivateKey.fromString = async function (arg) {
  $.checkArgument(_.isString(arg), 'No valid string was provided')
  return new HDPrivateKey(arg).initialized
}

HDPrivateKey.fromObject = async function (arg) {
  $.checkArgument(_.isObject(arg), 'No valid argument was provided')
  return new HDPrivateKey(arg).initialized
}

HDPrivateKey.prototype._buildFromJSON = async function (arg) {
  return this._buildFromObject(JSON.parse(arg))
}

HDPrivateKey.prototype._buildFromObject = async function (arg) {
...
HDPrivateKey.prototype._buildFromSerialized = async function (arg) {
  var decoded = await Base58Check.decode(arg)
  var buffers = {
...
HDPrivateKey.prototype._generateRandomly = async function (network) {
 ...
 * @return HDPrivateKey
 */
HDPrivateKey.fromSeed = async function (hexa, network) {
  ...
  var hash = await Hash.sha512hmac(hexa, buffer.Buffer.from('Bitcoin seed'))
 ...
 */
HDPrivateKey.prototype._buildFromBuffers = async function (arg) {
  ...
  if (!arg.checksum || !arg.checksum.length) {
    arg.checksum = await Base58Check.checksum(concat)
  } else {
    var checksum = await Base58Check.checksum(concat)
    if (arg.checksum.toString() !== checksum.toString()) {
  ...
  var privateKey = await new PrivateKey(BN.fromBuffer(arg.privateKey), network).initialized
  var publicKey = privateKey.toPublicKey()
  var size = HDPrivateKey.ParentFingerPrintSize
  var fingerPrint = await Hash.sha256ripemd160(publicKey.toBuffer()).slice(0, size)
 ...
 * @return {HDPrivateKey}
 */
HDPrivateKey.fromBuffer = async function (buf) {
  return new HDPrivateKey(buf.toString()).initialized
}
 ...
 * @return {HDPrivateKey}
 */
HDPrivateKey.fromHex = async function (hex) {
