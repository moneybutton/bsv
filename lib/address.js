...
function decodeCashAddress(address) {
  ...
  function validChecksum(cashAddrPrefix, payload) {
    ...
    var cashAddrPrefixData = cashAddrPrefixToArray(cashAddrPrefix).concat([0]);
    return polymod(cashAddrPrefixData.concat(payload)).eqn(0);
  }
  ...
  var cashAddrPrefix, encodedPayload;

  if (pieces.length === 2) {
    cashAddrPrefix = pieces[0];
  ...
  if (cashAddrPrefix) {
    $.checkArgument(validChecksum(cashAddrPrefix, payload), 'Invalid checksum:'+ address);
  } else {

    var netNames = ['livenet','testnet','regtest'];
    var i;

    while(!cashAddrPrefix && (i = netNames.shift())){
      var p  =  Networks.get(i).cashAddrPrefix;
      if(validChecksum(p, payload)) {
        cashAddrPrefix = p;
      }
    }
    $.checkArgument(cashAddrPrefix, 'Invalid checksum:'+ address);
  }
  ...
  var network = Networks.get(cashAddrPrefix);
//console.log('[address.js.336:network:]',network); //TODO

  var info = {};

  //return { cashAddrPrefix, type, hash };
...
Address.prototype.toCashAddress = function(stripcashAddrPrefix) {
  ...
  var cashAddrPrefixData = this.network.cashAddrPrefixArray.concat([0]);
  ...
  var checksumData = cashAddrPrefixData.concat(payloadData).concat(eight0);
  var payload = payloadData.concat(checksumToArray(polymod(checksumData)));
  if(stripcashAddrPrefix === true) {
    return base32.encode(payload);
  } else {
    return this.network.cashAddrPrefix+ ':' + base32.encode(payload);
  }
};
