...
function inject(deps) {
  ...
  Address.prototype = deps && deps.Struct ? Object.create(deps.Struct.prototype)
  ...
  Address.prototype.toScript = function() {
    let Script = this.constructor.Script;
    let type = this.type();
    let script;
    if (type === 'pubkeyhash') {
      script = Script();
  ...
  for (let dep of Object.keys(dependencies)) {
    Address[dep] = dependencies[dep];
  }
  if (deps) {
    for (let dep of Object.keys(deps)) {
      Address[dep] = deps[dep];
    }
  }
