 ...
 * =====================
 *
 * BIP39 is a way to turn a random seed into a string of words, and then that
 * string of words into a seed for a private key or BIP32 master private key.
 * It does not go the other way around (i.e., you cannot turn a private key
 * into a string of words). The way to use it is
 * words=BIP39.entropy2mnemoic(buf) and then BIP39.mnemonic2seed(words, passphrase)
 */
...
BIP39.mnemonic2seed = function(mnemonic, passphrase) {
  if (!passphrase)
    passphrase = "";
  var buf = KDF.PBKDF2(new Buffer(mnemonic), new Buffer("mnemonic" + passphrase), 2048, 64 * 8);
  return buf;
}
