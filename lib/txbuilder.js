...
let inject = function (deps) {
  ...
  /**
   * pubkey can be either a Pubkey in the case of a pubkeyhash transaction, or
   * a Script in the case of a p2sh transaction.
   */
  ...
  Txbuilder.prototype.fromScripthashMultisig = function (txhashbuf, txoutnum, txout, script) {
    ...
    this.txins.push(Txin().fromScripthashMultisigTxout(txhashbuf, txoutnum, txout, script))
    this.utxoutmap.add(txhashbuf, txoutnum, txout)
    return this
  }
   ...
   */
  Txbuilder.allSigsPresent = function (n, script) {
  ...
  Txbuilder.prototype.sign = function (i, keypair) {
    ...
    if (script.isPubkeyhashIn()) {
      ...
      let sig = this.tx.sign(keypair, Sig.SIGHASH_ALL, i, redeemScript)
      ...
      let n = pubkeybufs.length
      if (Txbuilder.allSigsPresent(n, script)) {
