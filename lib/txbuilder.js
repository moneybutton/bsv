...
let inject = function (deps) {
  ...
  let Txbuilder = function Txbuilder (tx, txins, txouts, utxoutmap, changeAddress, feePerKBNum, nlocktime, version) {
    if (!(this instanceof Txbuilder)) {
      return new Txbuilder(tx, txins, txouts, utxoutmap, changeAddress, feePerKBNum, nlocktime, version)
    }
    this.initialize()
    this.fromObject({tx, txins, txouts, utxoutmap, changeAddress, feePerKBNum, nlocktime, version})
  }
   ...
   */
  Txbuilder.prototype.fromScript = function (txhashbuf, txoutnum, txout, script, seqnum) {
    ...
    this.txins.push(Txin().fromObject({txhashbuf, txoutnum, seqnum}).setScript(script))
   ...
   * transaction.
   */
  Txbuilder.prototype.fromPubkeyhash = function (txhashbuf, txoutnum, txout, pubkey, seqnum) {
    ...
    this.txins.push(Txin().fromObject({seqnum}).fromPubkeyhashTxout(txhashbuf, txoutnum, txout, pubkey))
    this.utxoutmap.add(txhashbuf, txoutnum, txout)
    return this
  }
   ...
   * the transaction.
   */
  Txbuilder.prototype.fromScripthashMultisig = function (txhashbuf, txoutnum, txout, redeemScript, seqnum) {
    ...
    this.txins.push(Txin().fromObject({seqnum}).fromScripthashMultisigTxout(txhashbuf, txoutnum, txout, redeemScript))
    this.utxoutmap.add(txhashbuf, txoutnum, txout)
    return this
  }
  ...
  Txbuilder.prototype.build = function () {
    ...
    if (changeAmount.geq(shouldfeebn)) {
      ...
      if (changeAmount.lt(Constants.dustnum)) {
        throw new Error('unable to create change amount less than dust')
      }
  ...
  Txbuilder.prototype.getSig = function (keypair, nhashtype, nin, subscript) {
    return this.tx.sign(keypair, Sig.SIGHASH_ALL, nin, subscript)
  }
