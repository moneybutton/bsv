  ...
  if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
    ...
    switch (opcodenum) {
      ...
      case Opcode.OP_RSHIFT:
        ...
        let bnShifted = new BN(shifted.toArray().slice(bn1.byteLength() * -1))
        // this is not a bug! toScriptNumBuffer adds extra bytes that would make the tests fail. Have to use plain toBuffer
        this.stack.push(bnShifted.toBuffer())
        break
