 ...
 * ==============
 *
 * An abstraction over node's net module. Has the same interface as
 * network-browser. Makes it easy to have the same network interface whether in
 * node or the browser.
 */
"use strict";
let dependencies = {
...
function inject(deps) {
  ...
  Network.prototype.onConnection = function(bufstream) {
    ...
    this.awaits.forEach(function(await) {
      await.resolve(netbufs);
    });
    return this;
  };

  Network.prototype.netbufs = function*() {
  ...
  Network.prototype.connect = function(opts) {
    return new Promise(function(resolve, reject) {
      ...
      bufstream.on('connect', function() {
        let netbufs = Netbufs(opts, bufstream);
        netbufs.monitor();
        resolve(netbufs);
      });
