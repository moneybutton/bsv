...
let inject = function (deps) {
  ...
  Script.prototype.fromString = function (str) {
    ...
    while (i < tokens.length) {
      ...
      let opCodeNum
      try {
        let opCode = new OpCode().fromString(token)
  ...
  Script.prototype.fromBitcoindString = function (str) {
    ...
    for (i = 0; i < tokens.length; i++) {
      ...
      if (token[0] === '0' && token[1] === 'x') {
        ...
        let tbuf = new Script().writeBuffer(cbuf).toBuffer()
        bw.write(tbuf)
        ...
        let bn = new Bn(token)
        let script = new Script().writeBn(bn)
  ...
  Script.prototype.toBitcoindString = function () {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      let chunk = this.chunks[i]
      if (chunk.buf) {
        let buf = new Script([chunk]).toBuffer()
        let hex = buf.toString('hex')
        str = str + ' ' + '0x' + hex
      } else if (OpCode.str[chunk.opCodeNum] !== undefined) {
        let ostr = new OpCode(chunk.opCodeNum).toString()
  ...
  Script.sortPubKeys = function (pubKeys) {
    return pubKeys.slice().sort((pubKey1, pubKey2) => {
      let buf1 = pubKey1.toBuffer()
      let buf2 = pubKey2.toBuffer()
      let len = buf1.length > buf1.length ? buf1.length : buf2.length
      for (let i = 0; i <= len; i++) {
        if (buf1[i] === undefined) {
   ...
   * lexicographically.
  */
  Script.prototype.fromPubKeys = function (m, pubKeys, sort) {
    if (typeof m !== 'number') {
   ...
   */
  Script.prototype.isScripthashMultisigIn = function () {
  ...
  Script.prototype.isMultisigOut = function () {
    let m = this.chunks[0].opCodeNum - OpCode.OP_1 + 1
    if (!(m >= 1 && m <= 16)) {
      return false
    }
    let pubKeychunks = this.chunks.slice(1, this.chunks.length - 2)
    if (!pubKeychunks.every((chunk) => {
      try {
        let buf = chunk.buf
        let pubKey = new PubKey().fromDer(buf)
        pubKey.validate()
        return true
      } catch (err) {
        return false
    ...
    let n = this.chunks[this.chunks.length - 2].opCodeNum - OpCode.OP_1 + 1
    if (!(n >= m && n <= 16)) {
      return false
    }
  ...
  Script.prototype.isMultisigIn = function () {
    ...
    let remaining = this.chunks.slice(1)
  ...
  Script.prototype.findAndDelete = function (script) {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      let script2 = new Script([this.chunks[i]])
  ...
  Script.prototype.writeString = function (str) {
    let script = new Script().fromString(str)
    this.chunks = this.chunks.concat(script.chunks)
    return this
  }
  ...
  Script.prototype.setChunkOpCode = function (i, opCodeNum) {
  ...
  Script.prototype.setChunkBn = function (i, bn) {
  ...
  Script.prototype.setChunkBuffer = function (i, buf) {
