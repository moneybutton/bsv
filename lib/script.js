...
let inject = function (deps) {
  ...
  /**
   * Generate a multisig output script from a list of public keys.
  */
  Script.prototype.fromPubkeys = function (m, pubkeys) {
    if (typeof m !== 'number') {
  ...
  Script.prototype.isMultisigOut = function () {
    ...
    let pubkeychunks = this.chunks.slice(1, 1 + m)
    ...
    let n = this.chunks[1 + m].opcodenum - Opcode.OP_1 + 1
    if (!(n >= m && n <= 16)) {
      return false
    }
    if (this.chunks[1 + m + 1].opcodenum !== Opcode.OP_CHECKMULTISIG) {
      return false
    }
    return true
  }
