/**
 * Base58 Check Encoding
 * =====================
 *
 * Base58 check encoding. The usual way to use it is
 * Base58Check(buf).toString() or Base58Check(str).toBuffer(). This code was
 * originally written in BitcoinJS, then found its way into fullnode via
 * bitcore.
 */
"use strict";
let base58 = require('./base58');
let sha256sha256 = require('./hash').sha256sha256;
let cmp = require('./cmp');

let Base58Check = function Base58Check(obj) {
  if (!(this instanceof Base58Check))
    return new Base58Check(obj);
  if (Buffer.isBuffer(obj)) {
    let buf = obj;
    this.fromBuffer(buf);
  } else if (typeof obj === 'string') {
    let str = obj;
    this.fromString(str);
  } else if (obj) {
    this.fromObject(obj);
  }
};

Base58Check.prototype.fromObject = function(obj) {
  this.buf = obj.buf || this.buf || undefined;
  return this;
};

Base58Check.prototype.fromHex = function(hex) {
  return this.fromBuffer(new Buffer(hex, 'hex'));
};

Base58Check.prototype.toHex = function() {
  return this.toBuffer().toString('hex');
};

Base58Check.decode = function(s) {
  if (typeof s !== 'string')
    throw new Error('Input must be a string');

  let buf = base58.decode(s);

  if (buf.length < 4)
    throw new Error("Input string too short");

  let data = buf.slice(0, -4);
  let csum = buf.slice(-4);

  let hash = sha256sha256(data);
  let hash4 = hash.slice(0, 4);

  if (!cmp.eq(csum, hash4))
    throw new Error("Checksum mismatch");

  return data;
};

Base58Check.encode = function(buf) {
  if (!Buffer.isBuffer(buf))
    throw new Error('Input must be a buffer');
  let checkedBuf = new Buffer(buf.length + 4);
  let hash = sha256sha256(buf);
  buf.copy(checkedBuf);
  hash.copy(checkedBuf, buf.length);
  return base58.encode(checkedBuf);
};

Base58Check.prototype.fromBuffer = function(buf) {
  this.buf = buf;
  return this;
};

Base58Check.prototype.fromString = function(str) {
  let buf = Base58Check.decode(str);
  this.buf = buf;
  return this;
};

Base58Check.prototype.toBuffer = function() {
  return this.buf;
};

Base58Check.prototype.toString = function() {
  return Base58Check.encode(this.buf);
};

module.exports = Base58Check;
