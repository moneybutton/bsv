...
class Tx extends Struct {
   ...
   */
  sighash (nHashType, nIn, subScript, valueBn, flags = 0, hashCache = {}) {
    ...
    if (
      ...
      if (!(nHashType & Sig.SIGHASH_ANYONECANPAY)) {
        hashPrevouts = hashCache.hashPrevouts ? hashCache.hashPrevouts : hashCache.hashPrevouts = this.hashPrevouts()
      }
      ...
      if (
        ...
        hashSequence = hashCache.hashSequence ? hashCache.hashSequence : hashCache.hashSequence = this.hashSequence()
      }
      ...
      if (
        ...
        hashOutputs = hashCache.hashOutputs ? hashCache.hashOutputs : hashCache.hashOutputs = this.hashOutputs()
      } else if (
        (nHashType & 0x1f) === Sig.SIGHASH_SINGLE &&
  ...
  sign (keyPair, nHashType = Sig.SIGHASH_ALL | Sig.SIGHASH_FORKID, nIn, subScript, valueBn, flags = Tx.SCRIPT_ENABLE_SIGHASH_FORKID, hashCache = {}) {
    let hashBuf = this.sighash(nHashType, nIn, subScript, valueBn, flags, hashCache)
  ...
  verify (
    ...
    let hashBuf = this.sighash(sig.nHashType, nIn, subScript, valueBn, flags, hashCache)
