...
Tx.prototype.fromBufR = function(br) {
...
Tx.prototype.sighash = function(nhashtype, nin, subscript) {
  ...
  var buf = BufW().write(txcopy.toBuffer()).writeInt32LE(nhashtype).concat();
  return BufR(Hash.sha256sha256(buf)).readReverse();
};
...
Tx.prototype.id = function() {
  return BufR(this.hash()).readReverse();
};

Tx.prototype.addTxin = function(hashbuf, txoutnum, script, seqnum) {
  var txin;
  if (hashbuf instanceof Txin)
    txin = hashbuf;
  else
    txin = Txin(hashbuf, txoutnum, script, seqnum);
