...
let inject = function (deps) {
  ...
  Workers.prototype.asyncMethod = function (obj, methodname, args, id) {
    if (!args) {
      throw new Error('must specify args')
    }
    id = id !== undefined ? id : ++this.lastid
    let workersCmd = WorkersCmd().fromMethod(obj, methodname, args, id)
  ...
  Workers.asyncMethod = function (obj, methodname, args, id) {
    if (!globalWorkers) {
      globalWorkers = Workers().spawn()
    }
    return globalWorkers.asyncMethod.apply(globalWorkers, arguments)
  }
