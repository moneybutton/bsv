...
describe('#Ecies', function () {
  ...
  describe('@asyncBitcoreEncrypt', function () {
    it('should return a buffer', async function () {
      let encBuf = await Ecies.asyncBitcoreEncrypt(messageBuf, tokey.pubKey, fromkey)
      Buffer.isBuffer(encBuf).should.equal(true)
    })

    it('should return a buffer if fromkey is not present', async function () {
      let encBuf = await Ecies.asyncBitcoreEncrypt(messageBuf, tokey.pubKey)
      Buffer.isBuffer(encBuf).should.equal(true)
    })
  })
  ...
  describe('@bitcoreDecrypt', function () {
    it('should decrypt that which was encrypted', function () {
      let encBuf = Ecies.bitcoreEncrypt(messageBuf, tokey.pubKey, fromkey)
      let messageBuf2 = Ecies.bitcoreDecrypt(encBuf, tokey.privKey)
  ...
  describe('@asyncDecrypt', function () {
    it('should decrypt that which was encrypted', async function () {
      let encBuf = await Ecies.asyncBitcoreEncrypt(messageBuf, tokey.pubKey, fromkey)
      let messageBuf2 = await Ecies.asyncBitcoreDecrypt(encBuf, tokey.privKey)
      messageBuf2.toString('hex').should.equal(messageBuf.toString('hex'))
    })
  ...
  describe('Electrum Ecies', function () {
    let alicePrivKey = PrivKey.fromString('L1Ejc5dAigm5XrM3mNptMEsNnHzS7s51YxU7J61ewGshZTKkbmzJ')
    let aliceKeyPair = KeyPair.fromPrivKey(alicePrivKey)
    let bobPrivKey = PrivKey.fromString('KxfxrUXSMjJQcb3JgnaaA6MqsrKQ1nBSxvhuigdKRyFiEm6BZDgG')
    let bobKeyPair = KeyPair.fromPrivKey(bobPrivKey)
    ...
    let encrypted = 'QklFMQM55QTWSSsILaluEejwOXlrBs1IVcEB4kkqbxDz4Fap56+ajq0hzmnaQJXwUMZ/DUNgEx9i2TIhCA1mpBFIfxWZy+sH6H+sqqfX3sPHsGu0ug=='
    let encBuf = Buffer.from(encrypted, 'base64')
    ...
    it('retrieves senders PubKey from the encypted buffer', function () {
      let bob2 = Ecies().PrivKey(bobPrivKey)
      let decrypted = bob2.decrypt(encBuf).toString()
      bob2._PubKey.toDER().should.deep.equal(alicePrivKey.PubKey.toDER())
      decrypted.should.equal(message)
    })

    let message1 = 'This is message from first sender'
    let message2 = 'This is message from second sender'

    it('decrypt messages from different senders', function () {
      let sender1 = Ecies().PubKey(bobPrivKey.PubKey)
      let sender2 = Ecies().PubKey(bobPrivKey.PubKey)
      let bob2 = Ecies().PrivKey(bobPrivKey)
      let decrypted1 = bob2.decrypt(sender1.encrypt(message1)).toString()
      let decrypted2 = bob2.decrypt(sender2.encrypt(message2)).toString()
      decrypted1.should.equal(message1)
      decrypted2.should.equal(message2)
    })

    it('roundtrips', function () {
      let secret = 'some secret message!!!'
      let encrypted = alice.encrypt(secret)
      let decrypted = bob
        .decrypt(encrypted)
        .toString()
    ...
    it('correctly fails if trying to decrypt a bad message', function () {
      let encrypted = Buffer.from(encBuf)
    ...
    it('decrypting uncompressed keys', function () {
      ...
      // test uncompressed
      let alicePrivKey = bsv.PrivKey.fromObject({
        bn: '1fa76f9c799ca3a51e2c7c901d3ba8e24f6d870beccf8df56faf30120b38f360',
        compressed: false,
        network: 'livenet'
      })
      let alicePubKey = bsv.PubKey.fromPrivKey(alicePrivKey) // alicePrivKey.PubKey
      alicePrivKey.compressed.should.equal(false)

      let cypher1 = Ecies().PrivKey(alicePrivKey).PubKey(alicePubKey)
      let encrypted = cypher1.encrypt(secret)

      let cypher2 = Ecies().PrivKey(alicePrivKey).PubKey(alicePubKey)
